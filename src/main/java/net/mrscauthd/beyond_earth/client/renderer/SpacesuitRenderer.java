...
public class SpacesuitRenderer {
    ...
    private static Map<String, ModelPart> createModelMap(EquipmentSlot slot) {
        ...
        if (!map.containsKey("head")) map.put("head", empty);
        if (!map.containsKey("body")) map.put("body", empty);
        if (!map.containsKey("right_arm")) map.put("right_arm", empty);
        if (!map.containsKey("left_arm")) map.put("left_arm", empty);
        if (!map.containsKey("right_leg")) map.put("right_leg", empty);
        if (!map.containsKey("left_leg")) map.put("left_leg", empty);

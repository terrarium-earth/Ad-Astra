...
import net.mrscauthd.beyond_earth.gui.PlanetSelectionHandlerFactory;
...
public class RocketEntity extends VehicleEntity {
    ...
    private void explodeIfStopped() {
        if (this.getVelocity().getY() < ROCKET_MAX_SPEED / 10) {
            this.explode();
        }
    }
    ...
    public void explode() {
        if (!this.world.isClient) {
            ...
            for (int i = 0; i < 3; i++) {
                world.createExplosion(null, this.getX() + 0.5, this.getY() + 0.5, this.getZ() + 0.5, 7.0f * tierMultiplier, true, Explosion.DestructionType.DESTROY);
            }
        }
        this.remove(RemovalReason.DISCARDED);
    }

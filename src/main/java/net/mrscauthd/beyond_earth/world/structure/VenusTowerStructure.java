...
public class VenusTowerStructure extends StructureFeature<JigsawConfiguration> {

    public VenusTowerStructure(Codec<JigsawConfiguration> codec) {
        super(codec, (context) -> {
            if (!VenusTowerStructure.isFeatureChunk(context)) {
                return Optional.empty();
            } else {
                return VenusTowerStructure.createPiecesGenerator(context);
            }
    ...
    public static Optional<PieceGenerator<JigsawConfiguration>> createPiecesGenerator(PieceGeneratorSupplier.Context<JigsawConfiguration> context) {
        ...
        JigsawConfiguration newConfig = new JigsawConfiguration(() -> context.registryAccess().ownedRegistryOrThrow(Registry.TEMPLATE_POOL_REGISTRY)
                .get(new ResourceLocation(BeyondEarthMod.MODID, "run_venus_tower/side_venus_tower_start")), 25);

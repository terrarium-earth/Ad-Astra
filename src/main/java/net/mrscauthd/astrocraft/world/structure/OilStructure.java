...
public class OilStructure extends StructureFeature<JigsawConfiguration> {

    public OilStructure(Codec<JigsawConfiguration> codec) {
        super(codec, (context) -> {
            if (!OilStructure.isFeatureChunk(context)) {
                return Optional.empty();
            } else {
                return OilStructure.createPiecesGenerator(context);
            }
    ...
    public static Optional<PieceGenerator<JigsawConfiguration>> createPiecesGenerator(PieceGeneratorSupplier.Context<JigsawConfiguration> context) {
        ...
        JigsawConfiguration newConfig = new JigsawConfiguration(() -> context.registryAccess().ownedRegistryOrThrow(Registry.TEMPLATE_POOL_REGISTRY)
                .get(new ResourceLocation(AstroCraftMod.MODID, "run_oil/side_oil_start")), 25);

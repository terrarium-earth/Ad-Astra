...
public class FluidPipeBlock extends AbstractPipeBlock {
    ...
    public BlockState togglePipeState(BlockState state, EnumProperty<PipeState> property, PlayerEntity user) {
        ...
        switch (state.get(property)) {
            case NORMAL -> user.sendMessage(Text.translatable("info.ad_astra.normal"), true);
            case INSERT -> user.sendMessage(Text.translatable("info.ad_astra.insert"), true);
            case EXTRACT -> user.sendMessage(Text.translatable("info.ad_astra.extract"), true);
            case NONE -> user.sendMessage(Text.translatable("info.ad_astra.none"), true);
        }
        return state;
    }
}
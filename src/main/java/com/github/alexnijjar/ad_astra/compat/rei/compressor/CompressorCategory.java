...
public class CompressorCategory implements DisplayCategory<CompressorDisplay> {
    ...
    public List<Widget> setupDisplay(CompressorDisplay display, Rectangle bounds) {
        ...
        widgets.add(widget);
        widgets.add(Widgets.withTooltip(Widgets.withBounds(widget, bounds), new TranslatableText("rei.tooltip.ad_astra.energy_using", AdAstra.CONFIG.compressor.energyPerTick)));

        Text ratioText = new TranslatableText("rei.text.ad_astra.seconds", display.recipe().getCookTime() / 20);

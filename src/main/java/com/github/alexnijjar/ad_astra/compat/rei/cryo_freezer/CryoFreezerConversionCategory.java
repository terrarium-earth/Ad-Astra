...
public class CryoFreezerConversionCategory implements DisplayCategory<CryoFreezerConversionDisplay> {
	...
	public List<Widget> setupDisplay(CryoFreezerConversionDisplay display, Rectangle bounds) {
		...
		Text ratioText = new TranslatableText("rei.text.ad_astra.amount", 1000 * display.recipe().getConversionRatio());
		...
		widgets.add(widget);
		widgets.add(Widgets.withTooltip(Widgets.withBounds(widget, bounds), new TranslatableText("rei.tooltip.ad_astra.energy_using", AdAstra.CONFIG.cryoFreezer.energyPerTick)));

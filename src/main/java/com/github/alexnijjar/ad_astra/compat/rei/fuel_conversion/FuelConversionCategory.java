...
public class FuelConversionCategory implements DisplayCategory<FuelConversionDisplay> {
    ...
    public Text getTitle() {
        return new TranslatableText(ModBlocks.FUEL_REFINERY.getTranslationKey());
    }
    ...
    public List<Widget> setupDisplay(FuelConversionDisplay display, Rectangle bounds) {
        ...
        Text ratioText = new TranslatableText("rei.text.ad_astra.conversion_ratio", display.recipe().getConversionRatio() * 100.0);
        ...
        widgets.add(widget);
        widgets.add(Widgets.withTooltip(Widgets.withBounds(widget, bounds), new TranslatableText("rei.tooltip.ad_astra.energy_using", AdAstra.CONFIG.fuelRefinery.energyPerTick)));

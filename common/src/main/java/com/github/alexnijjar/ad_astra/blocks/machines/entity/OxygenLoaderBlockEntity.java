	...
	public void tick() {
		...
		if (!this.world.isClient) {
			...
			if (!insertSlot.isEmpty() && extractSlot.getCount() < extractSlot.getMaxCount()) {
				...
				FluidUtils.insertFluidIntoTank(this, tanks.getFluids().get(0), 0, 1, f -> ModRecipes.OXYGEN_CONVERSION_RECIPE.getRecipes(this.world).stream().anyMatch(r -> r.getFluidInput().equals(f.getFluid())));
			}

...
import earth.terrarium.adastra.common.networking.messages.ServerboundRequestStationsPacket;
...
public class RadioScreen extends BaseCursorScreen {
    ...
    protected static void renderScrollingString(GuiGraphics guiGraphics, Font font, Component text, int minX, int minY, int maxX, int maxY, int color) {
        ...
        if (i > k) {
            ...
            guiGraphics.drawString(font, text, minX - (int) g, (minY + maxY - 9) / 2 + 1, color);
            guiGraphics.disableScissor();
        } else {
            guiGraphics.drawCenteredString(font, text, (minX + maxX) / 2, (minY + maxY - 9) / 2 + 1, color);
        }
    }
    ...
    private static void saveConfigOnThread() {
        Minecraft.getInstance().tell(() -> AdAstra.CONFIGURATOR.saveConfig(ClientConfig.class));
    }

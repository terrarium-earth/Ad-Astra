...
public class OilRefineryBlockEntity extends RecipeMachineBlockEntity<RefiningRecipe> implements BotariumFluidBlock<WrappedBlockFluidContainer>, GeoBlockEntity {
    ...
    public static final RawAnimation TURN_OFF = RawAnimation.begin()
        .thenPlay("animation.model.turn.off")
        .thenPlayAndHold("animation.model.idle.off");

    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    private final long[] lastFluid = new long[2];
    private final long[] fluidDifference = new long[2];
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllerRegistrar) {
        controllerRegistrar.add(new AnimationController<>(this, state -> {
            if (cookTimeTotal == 0 || !canFunction()) return state.setAndContinue(TURN_OFF);
            else return state.setAndContinue(TURN_ON);
        }));
    }
    ...
    public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
        return new OilRefineryMenu(id, inventory, this);
    }

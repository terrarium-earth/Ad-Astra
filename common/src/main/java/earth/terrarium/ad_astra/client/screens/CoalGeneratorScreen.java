...
public class CoalGeneratorScreen extends AbstractMachineScreen<CoalGeneratorBlockEntity, CoalGeneratorScreenHandler> {
    ...
    protected void renderBg(PoseStack matrices, float delta, int mouseX, int mouseY) {
        ...
        GuiUtil.drawFire(matrices, this.leftPos + FIRE_LEFT, this.topPos + FIRE_TOP, this.machine.getCookTime(), this.machine.getCookTimeTotal());
    ...
    public void render(PoseStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (GuiUtil.isHovering(this.getFireBounds(), mouseX, mouseY)) {
            this.renderTooltip(matrices, Component.translatable("gauge.ad_astra.burn_time", this.machine.getCookTime(), this.machine.getCookTimeTotal()), mouseX, mouseY);
        }
    }

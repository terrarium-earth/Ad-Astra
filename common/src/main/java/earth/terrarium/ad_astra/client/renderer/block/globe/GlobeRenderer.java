...
public class GlobeRenderer implements BlockEntityRenderer<GlobeBlockEntity> {
    ...
    public static class ItemRenderer extends BlockEntityWithoutLevelRenderer {
        ...
        public void renderByItem(ItemStack stack, ItemTransforms.TransformType transformType, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
            BlockState state = Registry.BLOCK.get(Registry.ITEM.getKey(stack.getItem())).defaultBlockState();
    ...
    private static void render(BlockState state, float yaw, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        BakedModel blockModel = ClientPlatformUtils.getModel(Minecraft.getInstance().getModelManager(), new ResourceLocation(AdAstra.MOD_ID, "block/" + Registry.BLOCK.getKey(state.getBlock()).getPath() + "_cube"));
        try (var ignored = new CloseablePoseStack(poseStack)) {

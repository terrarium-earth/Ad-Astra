    ...
    public static Set<BlockPos> run(Level level, BlockPos start) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (runAdditionalChecks(level, state, pos)) {
                ...
                if (state.is(ModTags.BLOCKS_FLOOD_FILL)) continue;
                VoxelShape collisionShape = state.getCollisionShape(level, pos);
                if (!state.isAir() && !state.is(ModTags.PASSES_FLOOD_FILL) && !collisionShape.isEmpty() && isSideSolid(collisionShape, pair.getSecond(), state) && (isFaceSturdy(collisionShape, pair.getSecond(), state) || isFaceSturdy(collisionShape, pair.getSecond().getOpposite(), state))) {
                    continue;
                }
            }

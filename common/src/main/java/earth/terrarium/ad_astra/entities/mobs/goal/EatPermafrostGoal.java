	...
	public boolean canStart() {
		if (this.mob.getRandom().nextInt(this.mob.isBaby() ? 50 : 1000) != 0) {
			...
			if (PERMAFROST_PREDICATE.test(this.world.getBlockState(blockPos))) {
				...
				return this.world.getBlockState(blockPos.down()).isOf(ModBlocks.PERMAFROST.get());
			}
		}
	}
	...
	public void tick() {
		...
		if (this.timer == this.getTickCount(4)) {
			...
			if (PERMAFROST_PREDICATE.test(this.world.getBlockState(blockPos))) {
				...
				if (this.world.getBlockState(blockPos2).isOf(ModBlocks.PERMAFROST.get())) {
					if (this.world.getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
						this.world.syncWorldEvent(2001, blockPos2, Block.getRawIdFromState(ModBlocks.PERMAFROST.get().getDefaultState()));
